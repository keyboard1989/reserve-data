...
func GetConfig(kyberENV string, authEnbl bool, endpointOW string) *Config {
	...
	if os.Getenv("KYBER_ENV") == "simulation" {
		...
		fetcherRunner = fetcher.NewTickerRunner(3*time.Second, 2*time.Second, 3*time.Second, 5*time.Second, 5*time.Second, 5*time.Second, 3*time.Minute)
		statFetcherRunner = fetcher.NewTickerRunner(3*time.Second, 2*time.Second, 3*time.Second, 5*time.Second, 5*time.Second, 5*time.Second, 3*time.Minute)
	}
